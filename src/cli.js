...
import { auditCredentialVault, backupCredentials, configureCredential, credentialStatus, initializeCredentialPlaceholders, listCredentialDefinitions, migrateLegacyCredential, recoverCredential, removeCredential, rotateCredential, validateCredential } from "./credentials.js";
...
async function vaultCommand(root, args, options) { const [action = "audit", name] = args; if (action === "initialize") return initializeCredentialPlaceholders(root, options); if (action === "rotate") return credentialCommand(root, ["rotate", name], options); if (action === "recover") { if (!name) throw new Error("Usage: vault recover <credential> [--apply]"); return recoverCredential(root, name, options); } if (action === "migrate") { if (!name) throw new Error("Usage: vault migrate <credential> [--apply --yes]"); return migrateLegacyCredential(root, name, options); } if (action === "audit") return auditCredentialVault(root); throw new Error("Usage: vault <initialize|rotate|recover|migrate|audit> [credential]"); }
