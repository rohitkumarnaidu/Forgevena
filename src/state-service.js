...
export async function stateStatus(root) { const engine = new FileStateEngine(root); const documents = await Promise.all(DOCUMENTS.map((document) => engine.validate(document))); const journals = await journalStatus(engine); return { schemaVersion: REGISTRY_SCHEMA_VERSION, documents, journals, healthy: documents.filter((entry) => entry.error?.code !== "ENOENT").every((entry) => entry.valid) && journals.valid }; }
...
async function journalStatus(engine) {
  try {
    const operations = await engine.history();
    return { valid: true, operationCount: operations.length, pending: operations.filter((entry) => entry.status === "prepared").map((entry) => entry.operationId) };
  } catch (error) {
    return { valid: false, operationCount: 0, pending: [], error: { code: error.code ?? "STATE_JOURNAL_READ_FAILED", message: error.message } };
  }
}
