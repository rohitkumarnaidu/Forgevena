...
export async function verifySupplyChainReadiness(root) {
  const required = ["package-lock.json", ".github/workflows/security.yml", ".github/workflows/publish.yml", "SECURITY.md", "docs/security/THREAT_MODEL.md"];
  const missing = [];
  ...
  const publish = await readText(path.join(root, ".github/workflows/publish.yml"));
