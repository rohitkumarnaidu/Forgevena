...
Add a local content-addressed registry and resolver beneath existing provider, plugin, MCP, template, skill, workflow, and agent adapters.

```mermaid
flowchart LR
  U["User or governed automation"] --> P["Preview and policy"]
  P --> C["ForgeRegistry Protocol Foundation"]
